...
color backgroundColor = color(0); //CHANGE: set the background to black
Bouncer bouncer;
Bouncer bouncer2;
...
void draw() {
  ...
  bouncer.draw();
  bouncer2.draw();
  //CHANGE: added new methods allowing the bouncers to change to a chosen color when clicked
  bouncer.mouseClicked(color(0, 255, 0, 50));
  bouncer2.mouseClicked(color(255, 10, 150, 50));
}